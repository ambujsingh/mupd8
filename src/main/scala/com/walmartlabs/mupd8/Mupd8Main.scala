...
class SlateCache(val io : IoPool, val usageLimit : Long) {
  ...
  private def unLockedPut(skey : String, value : Slate, dirty : Boolean = true) = {
    ...
    oldVal map { p =>
      lru.remove(p.keyRef)
      currentUsage -= bytesConsumed(skey, p.slate)
    }
...
case class PerformerPacket(
  ...
  def run() {
    ...
    val optSlate = if (performer.mtype == Updater)
      Some { 
    ...
    while (true) {
      threadVect foreach { tls => {
        ...
        items.zipWithIndex.foreach { case((key,item),i) => {
          ...
          val suc = storeIo.write(colname, key.drop(colname.length + 3).getBytes, item.slate.toBytes())
          if (suc) {
            item.dirty = false
